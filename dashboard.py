...
def get_exchange():
    from crypto_trader.data import make_exchange

    local_cfg = load_config("ไม่มีไฟล์.yaml")
    local_cfg["exchange"]["name"] = "kraken"
    return make_exchange(local_cfg)


@st.cache_data(ttl=300, show_spinner=False)
def get_ohlcv(symbol: str) -> pd.DataFrame:
    from crypto_trader.data import fetch_ohlcv

    return fetch_ohlcv(get_exchange(), symbol, "1h", 300)


def get_market_data(symbols: tuple[str, ...]) -> dict[str, pd.DataFrame]:
    data = {}
    for sym in symbols:
        try:
            data[sym] = get_ohlcv(sym)
        except Exception:  # noqa: BLE001
            continue
    return data


def get_close_matrix(market_data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    data = {sym: df["close"] for sym, df in market_data.items() if "close" in df}
    if not data:
        return pd.DataFrame()
...
def market_snapshot_from_ohlcv(df: pd.DataFrame) -> dict:
    if df.empty or "close" not in df:
        return {"price": None, "chg_1h": None, "chg_24h": None, "chg_7d": None}
    close = pd.to_numeric(df["close"], errors="coerce").dropna()
    if close.empty:
        return {"price": None, "chg_1h": None, "chg_24h": None, "chg_7d": None}
    price = float(close.iloc[-1])
    chg_1h = float(close.iloc[-1] / close.iloc[-2] - 1) if len(close) > 2 else None
    chg_24h = float(close.iloc[-1] / close.iloc[-25] - 1) if len(close) > 25 else None
    chg_7d = float(close.iloc[-1] / close.iloc[-24 * 7 - 1] - 1) if len(close) > 24 * 7 else None
    return {"price": price, "chg_1h": chg_1h, "chg_24h": chg_24h, "chg_7d": chg_7d}
...
def build_symbol_rows(
    ...
    for sym in WATCHLIST:
        ...
        unreal = None
        if include_market:
            df_market = market_data.get(sym, pd.DataFrame())
            if not df_market.empty:
                snap = market_snapshot_from_ohlcv(df_market)
                price = snap["price"]
                chg_1h, chg_24h, chg_7d = snap["chg_1h"], snap["chg_24h"], snap["chg_7d"]
                if in_pos and price is not None and entry and amount:
...
if st.sidebar.button("🧹 ล้าง cache ราคาตลาด", use_container_width=True):
    st.cache_data.clear()
    st.sidebar.success("ล้าง cache ราคาตลาดแล้ว")
...
market_data: dict[str, pd.DataFrame] = {}
if include_market:
    with st.spinner("โหลดราคาตลาดจาก Kraken..."):
        market_data = get_market_data(tuple(WATCHLIST))
symbol_df = build_symbol_rows(state_data, trades, include_market, market_data)
...
if include_market:
    closes = get_close_matrix(market_data)
    if closes.empty:
