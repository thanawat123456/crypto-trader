...
## 🧪 ผลทดสอบครบวงจร 2 ปี (2024-06→2026-06, daily, หลัง slippage)
ทดสอบบนทั้งขาขึ้น (Q4/24 +47%, Q2/25 +30%) และขาลง — เทียบ buy&hold:

| เหรียญ | buy&hold ผล/DD | tsmom ผล/DD | rsi2 ผล/DD |
|--------|:---:|:---:|:---:|
| BTC | +7% / -51% | +1% / -34% | **+18% / -10%** |
| ETH | -48% / -68% | +31% / -36% | **+26% / -10%** |
| SOL | -48% / -76% | -35% / -44% | -1% / -21% |
| XRP | +152% / -69% | +10% / -59% | +46% / -14% |
| ADA | -57% / -87% | +25% / -39% | **+56% / -16%** |

**ข้อสรุปที่ถูกต้อง (แก้ความเข้าใจจากการดูแค่ขาลง):**
- **rsi2 มีเอดจ์ risk-adjusted จริง แม้หัก slippage** — ชนะ buy&hold เกือบทุกเหรียญ + drawdown น้อยกว่ามาก (-10~-21% vs -51~-87%)
- **คุณค่า = ป้องกันทุน** (รอดตลาดหมี) ไม่ใช่กำไรดิบเยอะสุด
- ในตลาดขาขึ้นล้วน: กลยุทธ์ active **ตามหลัง buy&hold** (ยอมเสียกำไรแลกความปลอดภัย)
- rsi2 เสถียรกว่า tsmom (DD ต่ำกว่า) → ยืนยัน rsi2 เป็น default ถูกต้อง
- ข้อแม้: sample เดียว (2 ปี/daily/5 เหรียญ), บอทจริงรัน 1h — หลักฐานทิศทาง ไม่ใช่การันตี

---

## 🔬 ผล PBO/CSCV (คำสั่ง `pbo` — generic, read-only) BTC/USD daily 2 ปี
ทำ CSCV ตามเปเปอร์ López de Prado/Bailey (หั่น 12 บล็อก สลับ IS/OOS 924 ชุด):

| โหมด | PBO | สรุป |
|------|-----|------|
| **strategies** (เลือกดีสุดใน 7 กลยุทธ์) | **36.3%** (logit +1.10) | ⚠️ ระวังพอควร — ยังไม่พิสูจน์ rsi2 |
| **ema grid** (จูน fast/slow) | **79.4%** (logit -0.74) | ❌ overfit หนัก — อย่าเชื่อ |

**ข้อสรุป (ยืนยันเปเปอร์เป๊ะ):**
- **จูนพารามิเตอร์ EMA overfit 79%** = ตัวอย่าง "crossing moving averages = charlatanism" ในเปเปอร์ตรง ๆ → **optimize/walkforward ที่จูน ema เชื่อไม่ได้**
- เลือก *ชนิด* กลยุทธ์ (rsi2 ฯลฯ) overfit น้อยกว่ามาก (36%) แต่ยังไม่ผ่านเกณฑ์ "พิสูจน์แล้ว" → ต้องมี paper/OOS เพิ่ม
- คำสั่ง `pbo` ใช้ได้ทุกกลยุทธ์ (generic) → แก้ข้อจำกัด walkforward ที่ผูกแค่ ema_cross
